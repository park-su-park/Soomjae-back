...
public class LikeService {
    ...
    @Transactional
    public LikeStatusResponse createLike(String postType, Long postId, UsernamePasswordUserDetails userDetails) {
    ...
    @Transactional
    public LikeStatusResponse deleteLike(String postType, Long postId, UsernamePasswordUserDetails userDetails) {
        validatePost(postType, postId);

        Like like = likeRepository.findByPostTypeAndPostIdAndMemberId(postType, postId,
                userDetails.getMember().getId())
            .orElseThrow(() -> new LikeNotFoundException(ErrorMessages.LIKE_NOT_FOUND_EXCEPTION_MESSAGE));
    ...
    public LikeStatusResponse readLikeStatus(String postType, Long postId, UsernamePasswordUserDetails userDetails) {
        validatePost(postType, postId);

        Long likeCount = likeRepository.countByPostTypeAndPostId(postType, postId);

        Boolean liked =  likeRepository.existsByPostTypeAndPostIdAndMemberId(postType, postId,
